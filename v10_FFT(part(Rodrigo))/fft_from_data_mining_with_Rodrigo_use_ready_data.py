...
def plot_amplitude(size, count, generate=False):
    ...
    factor = 1 / 5  # какую часть посчитанных данных мы берем
    for i in range(len(z1)):
        for j in range(len(z1[0])):
            middle = count / 2
            di = abs(i - middle)
            dj = abs(j - middle)
            if di ** 2 + dj ** 2 > (middle*factor) ** 2:  # Если мы находимся внутри круга с радиусом, равным count/2
                z1[i, j] = 0
                z2[j, i] = 0
    ...
    field_ampl_full, field_phase_full = loaddata(size, count, 2)
    # writedata(size, count, field1_ampl, field1_phase, 3)
    field1_ampl_full, field1_phase_full = loaddata(size, count, 3)

    delta_ampl = field_ampl - field_ampl_full
    print("delta_ampl = " + str(np.linalg.norm(delta_ampl)))
    delta1_ampl = field1_ampl - field1_ampl_full
    print("delta_ampl = " + str(np.linalg.norm(delta1_ampl)))
    ...
    plt.subplots(figsize=(8, 8))  # создаем поля для отрисовки
    plt.pcolor(x, y, delta_ampl, cmap='gray',
               label="Амплитуда")
    plt.show()

    plt.subplots(figsize=(8, 8))  # создаем поля для отрисовки
    plt.pcolor(x, y, delta1_ampl, cmap='gray',
               label="Амплитуда")
    plt.show()
